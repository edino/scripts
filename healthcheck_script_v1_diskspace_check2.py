...
def main():
    try:
        ...
        print(f"Script execution completed successfully. Logfile created at: {log_file}")
...
# This function will also get Tarballs for /log Core Dumps and Kernel Dumps on top of the Main function
def main2():
    try:
        nvram_output = subprocess.run(["nvram", "get", "#li.serial"], capture_output=True, text=True).stdout.strip()
        log_file = f"/var/tam_healthcheck_{nvram_output}-{get_timestamp()}.log"
...
# This function will also get CTR on top of Tarballs for /log Core Dumps and Kernel Dumps on top of the Main function
def main3():
    try:
        nvram_output = subprocess.run(["nvram", "get", "#li.serial"], capture_output=True, text=True).stdout.strip()
        log_file = f"/var/tam_healthcheck_{nvram_output}-{get_timestamp()}.log"
